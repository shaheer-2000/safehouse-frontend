...
const Jobs = (props) => {
    ...
    <div className="w-full h-full flex flex-col bg-secondary">
        <div className="h-20 px-10 border-b-2 border-gray-300 bg-white flex flex-row items-end justify-between">
            <div>
                <NavLink to={`/dashboard/${props.user}/users/managers`} className="tabLink" activeClassName="border-b-2 border-yellow-600">Jobs</NavLink>
                <NavLink to={`/dashboard/${props.user}/users/housemates`} className="tabLink" activeClassName="border-b-2 border-yellow-600">Employers</NavLink>
